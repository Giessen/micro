...
* `matchbrace`: show matching braces for '()', '{}', '[]' when the cursor
  is on a brace character or (if `matchbraceouter` is enabled) inside and next 
  to them.

    default value: `true`

* `matchbraceouter`: simulate I-beam cursor behavior (cursor located not on a
   ...
   to the immediate outside of the cursor instead. Also when jumping to the 
   matching brace, move the cursor either to the matching brace character or 
   to the character next to it, depending on whether the initial cursor position 
   was on the brace character or next to it (i.e. "inside" the braces).
   With `matchbraceouter` disabled, micro will only match the brace directly
